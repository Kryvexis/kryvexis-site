...
const roadmap = [
  { stage: "Now", title: "Quotes module", desc: "Create professional quotes and convert to invoices with one click.", pills: ["Templates", "Discounts", "Quick convert"] },
  { stage: "Now", title: "Top sales per client", desc: "Instant view of best customers and what they buy most.", pills: ["Client ranking", "Repeat buyers", "Quick filters"] },
  { stage: "Next", title: "Client purchase history summary", desc: "See purchase history across the last few months — without exporting to spreadsheets.", pills: ["Monthly summary", "Item breakdown", "Printable"] },
  { stage: "Next", title: "One‑click print", desc: "Print quotes and invoices in a clean layout — fast, consistent, and staff friendly.", pills: ["A4 layout", "Branding", "PDF"] },
  { stage: "Soon", title: "Automatic PDF invoice saving", desc: "Auto-save PDFs for every invoice for easy record keeping.", pills: ["Auto naming", "Cloud ready", "Searchable"] },
];
...
{/* PRODUCT */}
      ...
      {/* ROADMAP */}
      <section id="roadmap" className="section relative flow-section snap-section">
        <Container>
          <Reveal>
            <SectionHeading
              kicker="Roadmap"
              title="What we’re building next"
              desc="Focused upgrades that make Kryvexis OS feel like real software for daily operations."
            />
          </Reveal>
          <Reveal delay={0.08}>
            <Spotlight className="mt-10 grid md:grid-cols-2 gap-5">
              {roadmap.map((it, i)=>{
                const stageTone = it.stage === "Now" ? "badge" : (it.stage === "Next" ? "badge" : "badge");
                return (
                  <Reveal key={it.title} delay={0.05 + i*0.04}>
                    <Tilt className="relative rounded-2xl">
                      <div className="glass rounded-2xl p-6 shadow-soft">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="text-lg font-extrabold text-white/95">{it.title}</div>
                            <div className="mut mt-2 text-sm leading-relaxed">{it.desc}</div>
                          </div>
                          <div className={stageTone}>{it.stage}</div>
                        </div>
                        <div className="mt-5 flex flex-wrap gap-2">
                          {it.pills.map(p=> <span key={p} className="badge">{p}</span>)}
                        </div>
                      </div>
                    </Tilt>
                  </Reveal>
          ...
          <div className="mt-8 text-sm text-white/60">Want to influence the roadmap? Join early access and tell us what your team needs most.</div>
        </Container>
      </section>
